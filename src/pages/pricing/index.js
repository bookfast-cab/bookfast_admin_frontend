...
const HelplineSettings = () => {
  const [platform_commission, setplatform_commission] = useState("");
  const [cancellation_penalties, setcancellation_penalties] = useState("");
  ...
  const fetchHelpline = async (accessToken) => {
    try {
      ...
      if (result.success && result.data) {
        setplatform_commission(result.data.platform_commission || "");
        setcancellation_penalties(result.data.cancellation_penalties || "");
      } else {
  ...
  const handleSave = async () => {
    if (!platform_commission.trim()) {
      setErrorMessage("Platform commission cannot be empty.");
      
      return;

    }

    if (!cancellation_penalties.trim()) {
      setErrorMessage("Cancellation penalties cannot be empty.");
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/charges`, {
        ...
        body: JSON.stringify({ platform_commission: platform_commission, cancellation_penalties: cancellation_penalties }),
      });
      ...
      <Grid item xs={12}>
        <Card sx={{ p: 3, boxShadow: 3, borderRadius: 2 }}>
          <TextField
            ...
            onChange={(e) => setplatform_commission(e.target.value)}
            sx={{ mb: 3 }}
          />
          ...
          <TextField
            ...
            variant="outlined"
            value={cancellation_penalties}
            onChange={(e) => setcancellation_penalties(e.target.value)}
            sx={{ mb: 3 }}
          />
