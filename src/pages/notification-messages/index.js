...
import TableBasic from 'src/views/tables/NotificationMessagesTable'
...
const MUITable = () => {
  ...
  const getNotificationMessages = async (page_num) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/notification-messages?${queryParams}`, {
      method: 'GET',
      headers: {
    ...
    <Grid container spacing={6}>
      ...
      <Grid item xs={6} style={{ textAlign: 'right' }}>
        <Button
          ...
          variant="contained"
          onClick={() => router.push('/notification-messages/add')}
        >
          Add
        </Button>
      </Grid>
