...
const MUITable = () => {
  ...
  const handleApprove = (id) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/approve-pending-membership`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: token,
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (params) => (
        <span>{params.row.plan_name || 'N/A'}</span>
      )
    },
    {
      field: 'start_date',
    ...
    },
    {
      ...
      flex: 0.8,
      renderCell: (params) => (
        <span style={{ fontWeight: 'bold' }}>₹{params.row.price || 'N/A'}</span>
      )
    },
