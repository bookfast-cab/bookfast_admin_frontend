...
const MUITable = () => {
  ...
  const getPromoCoupon = async (page_num) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/promoCoupon?${queryParams}`, {
      method: 'GET',
      headers: {
      ...
      .then((data) => {
        ...
        const rowsWithId = data.data.map((item) => ({
          ...item,
          id: item.coupon_id,
        }));
  ...
  const exportToExcel = (type = "") => {
    ...
    try {
      fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/exportCoupons?${queryParams.toString()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${token}`
  ...
  const columns = [
    {
      ...
      headerName: '#',
      width: 50,
      renderCell: (params) => params.row.coupon_id
    },

    { field: 'code', headerName: 'Code', width: 100 },
    { field: 'discount_type', headerName: 'Type', width: 80 },
    { field: 'discount_value', headerName: 'Discount Value', width: 80 },
    { field: 'min_ride_amount', headerName: 'Min. Ride Amount', width: 100 },
    {
      field: 'isSelectedCustomer',
      headerName: 'Special Customers',
      width: 100,
      renderCell: (params) => (params.row.isSelectedCustomer === 1 ? 'Yes' : '')
    },
    {
      field: 'valid_from',
      ...
      width: 140,
      renderCell: (params) =>
        new Date(params.row.valid_from).toLocaleDateString()
    },
    {
      field: 'valid_to',
      headerName: 'Valid To',
      width: 140,
      renderCell: (params) =>
        new Date(params.row.valid_to).toLocaleDateString()
    },

    { field: 'totalRedeemed', headerName: 'Redeemed', width: 120 },
  ...
  return (
    <Grid container spacing={4} sx={{ bgcolor: "white", padding: 3 }}>
      ...
      <Grid item xs={6}>
        <Typography variant="h5">Promo Coupon</Typography>
      </Grid>
      <Grid
        item
        xs={12}
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <Button startIcon={(
            ...
            variant="contained"
            onClick={() => router.push('/promo-coupon/add')}
          >
            Add
          </Button>
        </div>
        ...
        <Card>
          <CommonDataTable
            ...
            onUpdateStatus={(id, newStatus) =>
              setData((prevData) =>
                prevData.map((item) =>
                  item.coupon_id === id ? { ...item, status: newStatus } : item
                )
              )
            }
