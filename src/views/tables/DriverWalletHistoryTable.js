...
const DriverWalletHistoryTable = (props) => {
  const {
    items = [],
    totalRecords,
    totalPages,
          ...
          <TableBody>
            {items && items.map((item,index) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.driver?.driverName}</TableCell>
                ...
                <TableCell>{ item?.action=='cr' ? 'Credit' : 'Debit' }</TableCell>
                <TableCell className="whitespace-nowrap">{item?.created_at && formatDate(item?.created_at) || 'N/A'}</TableCell>
                <TableCell>
                  <IconButton
...
export default DriverWalletHistoryTable;
