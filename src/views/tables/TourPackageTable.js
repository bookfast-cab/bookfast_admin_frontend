...
const TourPackageTable = ({ items = [], onDelete,totalRecords, totalPages, currentPage, perPage, onPageChange = () => {} }) => {
  ...
  const handleEdit = (id) => {
    router.push(`/tourPackages/edit?id=${id}`);
  };
        ...
        <Table sx={{ minWidth: 650 }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell>location Covered</TableCell>
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{stripHtml(item.location_covered)}</TableCell>
                <TableCell>{item.package_cost || 'N/A'}</TableCell>
                <TableCell>
                  <IconButton
