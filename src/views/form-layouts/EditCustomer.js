...
const EditCustomerDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/customers/${id}`, {
        method: 'PUT',
        headers: {
  ...
  const fetchCustomerDetails = async () => {
    if (id) {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/customer/${id}`, {
                headers: {
                    Authorization: `${token}`,
                },
            });
            ...
            if (data.success) {
                setFormData({
                    ...
                    profile_picture: `${process.env.NEXT_PUBLIC_API_BASE_URL}/uploads/${data?.data.profile_picture || '' }`,
                });
                setProfilePic(`${process.env.NEXT_PUBLIC_API_BASE_URL}/uploads/${data?.data.profile_picture || '' }`);
            }
        } catch (error) {
