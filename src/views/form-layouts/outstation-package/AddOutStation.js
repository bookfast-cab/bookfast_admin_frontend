...
const AddOutStation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/outstation-package`, {
        method: 'post',
        headers: {
          Authorization: `${token}`,
...
<Box sx={{ display: "flex", gap: 2, flexWrap: "wrap", marginTop: "15px", marginBottom: "15px" }}>
  ...
  <Box sx={{ flex: 1 }}>
    ...
    <Select
      ...
      displayEmpty
    >
      <MenuItem value="" disabled>Select Pickup Location</MenuItem>
    ...
    {formErrors.pickup_location && (
      <FormHelperText error>{formErrors.pickup_location}</FormHelperText>
    )}
  </Box>
  ...
  <Box sx={{ flex: 1 }}>
    ...
    <Select
      ...
      displayEmpty
    >
      <MenuItem value="" disabled>Select Drop Location</MenuItem>
      {pickDropList.map((item) => (
        <MenuItem key={item.id} value={item.id}>
          {item.name}
        </MenuItem>
    ...
    {formErrors.drop_location && (
      <FormHelperText error>{formErrors.drop_location}</FormHelperText>
    )}
  </Box>
